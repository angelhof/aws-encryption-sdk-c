...
bool aws_cryptosdk_frame_is_valid(const struct aws_cryptosdk_frame *const frame) {
    ...
    /* TODO: Add a check for the size of iv */
    bool iv_size_valid = true;

    bool authtag_byte_buf_valid = aws_byte_buf_is_valid(&frame->authtag);
    bool authtag_byte_buf_static = frame->authtag.allocator == NULL;
    /* TODO: Add a check for the size of authtag */
    bool authtag_size_valid = true;
    ...
    return iv_byte_buf_valid && iv_byte_buf_static && iv_size_valid &&
        ...
        ciphertext_valid && ciphertext_static && ciphertext_size_valid;
        /* && sequence_number_in_range; */
}
